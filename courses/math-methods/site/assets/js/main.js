(() => {
  ...
  //
  // They start paused where the reader has asked for less motion, and playing
  // otherwise, so nothing is taken away from anyone who wants the animation.
  function addAnimationToggle(image) {
    // The control cannot go inside a link -- a button nested in an anchor is
    // invalid markup and traps the keyboard -- so for the project cards, whose
    // preview sits inside one, it goes after the card instead.
    const figure = image.closest("figure");
    const link = image.closest("a");
    const host = figure && !link ? figure : null;
    const container = host || (link ? link.parentElement : image.parentElement);
    // Guard on the image, not the container: the project cards all share one
    // grid, so a container-level check would fit a control to the first card
    // only and leave the other three playing with no way to stop them.
    ...
    function freeze() {
      ...
      try {
        ...
        return false; // Tainted canvas; leave the animation running.
      }
      return true;
    }
    ...
    function setPlaying(next) {
      if (next) {
        // Re-assigning the src restarts the GIF from its first frame.
        image.src = source;
        image.hidden = false;
        still.hidden = true;
      } else if (freeze()) {
